...
-- Reference: https://postgresql.verite.pro/blog/2024/07/15/uuid-v7-pure-sql.html

-- uuidv7 function to generate a uuidv7 value with sub-millisecond precision
-- Includes a 12-bit sub-millisecond timestamp fraction immediately after the timestamp
...
-- uuidv7_ms function to generate a uuidv7 value with millisecond precision
CREATE FUNCTION uuidv7_ms(timestamptz DEFAULT clock_timestamp()) RETURNS uuid
AS $$
...
COMMENT ON FUNCTION uuidv7_ms(timestamptz) IS
...
DROP FUNCTION IF EXISTS uuidv7_ms(timestamptz);
